...
After running the code, I found out that the longest movie was 'The Cure for Insomnia,' with 5,220 minutes. Whilst the oldest movie was the 'Black Smith Scene,' in 1893. 
...
After running the code above, I figured out that there are 1,248 movies made in 1995. 
...
After running the code above, I figured out that there are four movies that fall under the category. 
...
After running the code above, I have found out that there are three movies of Harry Potter, including 'The Sorcerer's Stone,' 'The Chamber of Secrets,' and 'Prisoner of Azkaban.' The movie with the highest rating is the 'Prisoner of Azkaban,' with the rating of 7.8. The movie with the highest votes was the 'Sorcerer's Stone' with 46,741 votes.
...
```{r}
movies %>%
  filter(Comedy ==1 | Animation == 1, votes >= 100, budget>0) %>%
  arrange(desc(budget))
```
After running the code above, I figured out that the highest budget for the movie is 'Wild Wild West,' with $170 million. 
